...
def main():
    ...
    for fname, image in zip(tqdm(args.img),image_list):
        ...
        if args.saveto is not None and args.ext == 'png':
            # Αποθήκευση εικόνας με wireframe σε PNG
            fig, ax = plt.subplots()
            ax.imshow(image_cp, cmap='gray')
            painter.draw_wireframe(ax, outputs)  # Χρήση του ax για το wireframe

            # Αποθήκευση εικόνας
            png_outpath = osp.join(args.saveto, pname.with_suffix('.png').name)
            plt.savefig(png_outpath, bbox_inches='tight')
            plt.close(fig)


        if args.saveto is not None and args.ext == 'txt':
            outpath = osp.join(args.saveto, pname.with_suffix('.txt').name)
            with open(outpath, 'w') as f:
                #f.write("Wireframe Results\n")
                #f.write("Junctions (Predicted):\n")
                #for junc in outputs['juncs_pred']:
                #    f.write(f"{junc[0]}, {junc[1]}\n")
                
                f.write("Lines (Predicted):\n")
                for line in outputs['lines_pred']:
                   f.write(f"{line[0]},{line[1]},{line[2]},{line[3]}\n")
                
                #f.write("\nJunction Scores:\n")
                #for score in outputs['juncs_score']:
                #    f.write(f"{score}\n")
                
                #f.write("\nLine Scores:\n")
                #for score in outputs['lines_score']:
                #    f.write(f"{score}\n")
